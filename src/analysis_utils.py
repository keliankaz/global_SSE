...
def center_sequences(
    ...
    # time window
    delta_t = (
        earthquakes.catalog.time.values.reshape(1, -1)      # 1, N_earthquake
        - slowslipevents.catalog.time.values.reshape(-1, 1) # N_slowslip, 1
    ) / np.timedelta64(1, "D")                              # N_slowslip, N_earthquake
    if use_durations is True:
        delta_t = delta_t/(slowslipevents.catalog.duration.values.reshape(-1,1)/(24*60*60))
    ...
    for i, ((_, SSE), i_window) in enumerate(zip(slowslipevents, is_in_window)):
        if np.any(i_window):
            ...
            xy_coordinates = np.column_stack((east, north))
            xy_coordinates = xy_coordinates - np.array([sse_east, sse_north])
            if use_dimensions is True:
                sse_dimension = Scaling.magnitude_to_size(
                    SSE.mag, SSE._stress_drop, "m"
                )
                xy_coordinates = xy_coordinates / sse_dimension
            ...
            xy_rotated = (rotation_matrix @ xy_coordinates.T).T
